...
Binary release entries are configured for macOS arm64 and macOS Intel. After the `v0.1.0` release assets exist:
...
The current manifest points at the `v0.1.0` release asset names. After the first tagged release, replace the temporary `checksum: "SKIP"` entries in `manifest.json` with the SHA-256 values generated by the workflow before announcing public installs. Do not reuse a published tag with a different binary after users can install it.
