...
The current manifest points at the `v0.1.1` release assets and includes their SHA-256 checksums. For future releases, update `manifest.json` with the checksums generated by the tag workflow before announcing public installs. Do not reuse a published tag with a different binary after users can install it.
