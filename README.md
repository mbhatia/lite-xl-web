...
- Detaches the native webview when the tab is inactive, preserving the webview object for later show/focus.
...
Binary releases are published for macOS arm64, macOS Intel, Linux x64, and Linux arm64:
...
`web` depends on the `web_lxl` library addon. LPM downloads the release asset matching the current architecture and installs it as `libraries/web_lxl/init.lib` on macOS or `libraries/web_lxl/init.so` on Linux, which is what the Lua plugin requires.

The Linux binaries currently load the plugin with the stub backend; a native Linux webview backend is not implemented yet. Windows binary packages are not published yet.
...
The `release-binaries` GitHub Actions workflow builds on native macOS arm64, macOS Intel, Linux x64, and Linux arm64 runners. It checks out Lite XL `v2.1.8` for `lite_xl_plugin_api.h`, builds and tests the native module, stages `dist/web_lxl.<arch>.<ext>`, writes `dist/web_lxl.<arch>.<ext>.sha256`, and uploads both as workflow artifacts. Tag pushes matching `v*` also upload those files to the GitHub release.

The current manifest points at the `v0.1.4` release asset names. After the tag workflow runs, replace the temporary `checksum: "SKIP"` entries in `manifest.json` with the SHA-256 values generated by the workflow before announcing public installs. Do not reuse a published tag with a different binary after users can install it.
